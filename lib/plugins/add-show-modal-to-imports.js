...
module.exports = function addShowModalToImports({ nodePath, imports, projectNameSpace, fileName, utils }) {
  ...
  showModalPaths.forEach((currentPath) => {
    ...
    if (args.length > 1) {
      ...
      if (templatePath.type === 'Literal') {
        ...
        let templateFile = utils.resolveFilePath(`${projectNameSpace}/templates/${templateName}`);

        if (templateFile) {
          imports.push(templateFile);
        }

        let associatedControllerFile = utils.resolveFilePath(`${projectNameSpace}/controllers/${templateName}`);
