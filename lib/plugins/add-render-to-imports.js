...
module.exports = function addRenderToImports ({ imports, nodePath, projectNameSpace, fileName, utils }) {
  ...
  renderPaths.forEach((currentPath) => {
    ...
    args.forEach((arg) => {
      if (arg.type === 'Literal') {
        let templateFile = utils.resolveFilePath(`${projectNameSpace}/templates/${arg.value}`);
        ...
        if (controllerPath && controllerPath.getValueProperty('value').type === 'Literal') {
          ...
          let controllerFile = utils.resolveFilePath(`${projectNameSpace}/controllers/${controllerName}`);
